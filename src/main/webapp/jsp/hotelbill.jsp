...
<head>
    ...
    <title>AirSystem-酒店订单</title>
    <link rel="stylesheet" href="http://cdn.static.runoob.com/libs/bootstrap/3.3.7/css/bootstrap.min.css">
</head>
<body>
<form id="hotelBillForm" method="post" action="${pageContext.request.contextPath}/hotel/bill/refund.html">
            ...
            <div class="col-lg-10">
                <div class="panel panel-primary">
                    <div class="panel-title">
                        <div style="text-align: center; font-size: 24px">酒店订单</div>
                    </div>
                    <div class="panel-body" style="text-align: center; margin-top: 100px; margin-bottom: 100px">
                        <div class="row">
                            <table id="tabHotelBill" class="table table-hover table-bordered">
                                <input type="hidden" id="hotelBillId" name="hotelBillId"/>
                                ...
                                <c:forEach var="hotelBill" items="${hotelBillList}" varStatus="i">
                                    <tr class="choose" hotelBillId="${hotelBill.id}">
                                        <input type="hidden" id="hotelBillId${i.count}" name="hotelBillId${i.count}"/>
                                        <td>${hotelBill.hotel.name}</td>
                                        <td>${hotelBill.hotel.address}</td>
                                        <td>${hotelBill.hotel.fullAddress}</td>
                                        <td>${hotelBill.checkInDate}</td>
                                        <td>${hotelBill.checkOutDate}</td>
                                        <td>${hotelBill.passenger.name}</td>
                                        <td>${hotelBill.date}</td>
                                    </tr>
                                </c:forEach>
                            </table>
                        </div>
...
<script src="${pageContext.request.contextPath}/js/hotelbill.js" type="text/javascript"></script>
</body>
</html>
