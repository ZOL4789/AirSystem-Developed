...
<head>
    ...
    <title>AirSystem-所有乘客信息</title>
    <link rel="stylesheet" href="http://cdn.static.runoob.com/libs/bootstrap/3.3.7/css/bootstrap.min.css">
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/common.css"/>
</head>
<body>
<form id="passengerForm" method="post" action="/AirSystem/passenger/sys/listAll.html">
            ...
            <div class="col-lg-10">
                <div class="panel panel-primary">
                    <div class="panel-title">
                        <div style="text-align: center; font-size: 24px">所有乘客</div>
                    </div>
                        ...
                        <div class="row">
                            <table id="tabHotel" class="table table-bordered">
                                ...
                                <c:forEach var="passenger" items="${passengerList}" varStatus="i">
                                    <tr>
                                        <td>姓名：<input style="width:100px" type="text" value="${passenger.name}"/></td>
                                        <td>身份证号：<input type="text" value="${passenger.IDNum}"/></td>
                                        <td>备注：
                                            <select id="selRoleName">
                                            <c:forEach var="name" items="${roleNameList}" varStatus="i">
                                                <c:choose>
                                                    <c:when test="${name == passenger.role.name}">
                                        ...
                                        <td>绑定用户：
                                            ...
                                            <c:forEach var="user" items="${userList}" varStatus="i">
                                                <c:choose>
                                                    <c:when test="${user.name == passenger.user.name}">
                                        ...
                                        <td>绑定时间：${passenger.date}</td>
                                        <td>
                                            <input passengerId="${passenger.id}"
                                                   passengerName="${passenger.name}"
                                                   passengerIDNum="${passenger.IDNum}"
                                                   passengerRoleName="${passenger.role.name}"
                                                   passengerUserName="${passenger.user.name}"
                                                   class="update" type="button" value="更新"/>
                                            <input class="delete" userId="${user.id}" type="button" value="删除"/>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </table>
...
<script src="${pageContext.request.contextPath}/js/admin/passengerlist.js" type="text/javascript"></script>
</body>
</html>
